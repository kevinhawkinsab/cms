...
const { data: postsData } = await useAsyncData(
  () => `article-list-block-posts-${props.block.id}`,
  async () => {
    try {
      const response = await $fetch('/GetPosts_CMS/posts', {
        baseURL: apiBase
      })
      return normalizePostList(response).filter(isPublicBlogPost)
    } catch (error) {
      console.warn('[ArticleListBlock] No se pudieron cargar posts en SSR:', error?.message || error)
      return []
    }
  },
  { default: () => [] }
